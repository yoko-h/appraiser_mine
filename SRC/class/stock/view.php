      ...
      <table border="0" cellpadding="2" cellspacing="0">
        ...
        <tr>
          ...
          <td>
            ...
            for ($i = 0; $i < 4; $i++) {
            ?>
              <input type="checkbox" name="sDistance[]" value="<?php print $i + 1; ?>"
                <?php
                // for ($j = 0; $j < 4; $j++) {
                // if ($param["sDistance"][$j] == $i + 1) print ' checked="checked"';
                if (isset($param["sDistance"]) && is_array($param["sDistance"]) && in_array($i + 1, $param["sDistance"])) {
        ...
        <tr>
          ...
          <td>
            ...
            for ($i = 0; $i < 5; $i++) {
            ?>
              <input type="checkbox" name="sRank[]" value="<?php print $i + 1; ?>"
                <?php
                // for ($j = 0; $j < 5; $j++) {
        ...
        <tr>
          ...
          <td>
            ...
            for ($i = 0; $i < 6; $i++) {
            ?>
              <input type="checkbox" name="sHow[]" value="<?php print $i + 1; ?>"
                <?php
                // for ($j = 0; $j < 6; $j++) {
                // if ($param["sHow"][$j] == $i + 1) print ' checked="checked"';
                if (isset($param["sRank"]) && is_array($param["sRank"]) && in_array($i + 1, $param["sRank"])) {
                  print ' checked="checked"';
    ...
    <table border="0" cellpadding="5" cellspacing="1">
      ...
      <tr>
        ...
        <td>
          <?php
          // if (!$param["stockNo"]) {
          if (!isset($param["stockNo"]) || !$param["stockNo"]) {
            $param["distance"] = 1;
          }
          for ($i = 0; $i < 4; $i++) {
            $check = '';
            // if (($param["distance"] - 1) == $i) {
            if ((($param["distance"] ?? 1) - 1) == $i) {
              $check = 'checked = "checked"';
            }
          ?>
      ...
      <tr>
        <th>内見</th>
        <td><input type="text" name="visitDT" value="<?php print $param["visitDT"] ?? '' ?>" />
