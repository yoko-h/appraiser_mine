...
function fnSqlGuideList($flg, $param)
{
  switch ($flg) {
    ...
    case 1:
      $select = "SELECT A.GUIDENO, A.CONTENT, A.ARTICLENO, A.NAME, A.FAX, A.RESULT, A.ACCEPT, A.EXAM, A.PURCHASE, B.ARTICLE, B.ROOM,"
        ...
        . "IF(STR_TO_DATE(A.GUIDESTARTDT, '%Y-%m-%d') IS NOT NULL, DATE_FORMAT(A.GUIDESTARTDT, '%Y/%m/%d'), '') AS GUIDESTARTDT,"
        // . "IF( A.GUIDEENDDT > '0000-00-00', DATE_FORMAT( A.GUIDEENDDT, '%Y/%m/%d'), '') AS GUIDEENDDT,"
        . "IF(STR_TO_DATE(A.GUIDEENDDT, '%Y-%m-%d') IS NOT NULL, DATE_FORMAT(A.GUIDEENDDT, '%Y/%m/%d'), '') AS GUIDEENDDT,"
        ...
        . "IF(STR_TO_DATE(A.ACCEPTDT, '%Y-%m-%d') IS NOT NULL, DATE_FORMAT(A.ACCEPTDT, '%Y/%m/%d'), '') AS ACCEPTDT";
  ...
  // if ($param["sAcceptDT"]) {
  if (!empty($param["sAcceptDT"]) && $param["sAcceptDT"] != '0000-00-00') {
...
function fnSqlGuideEdit($guideNo)
{
  $sql = "SELECT CONTENT, ACCEPT, EXAM, PURCHASE, GUIDESTARTTM, GUIDEENDTM,"
    ...
    . "IF(STR_TO_DATE(GUIDESTARTDT, '%Y-%m-%d') IS NOT NULL, DATE_FORMAT(GUIDESTARTDT, '%Y/%m/%d'), '') AS GUIDESTARTDT,"
    . "IF(STR_TO_DATE(GUIDEENDDT, '%Y-%m-%d') IS NOT NULL, DATE_FORMAT(GUIDEENDDT, '%Y/%m/%d'), '') AS GUIDEENDDT,"
    . "IF(STR_TO_DATE(ACCEPTDT, '%Y-%m-%d') IS NOT NULL, DATE_FORMAT(ACCEPTDT, '%Y/%m/%d'), '') AS ACCEPTDT";
