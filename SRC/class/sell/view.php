  ...
  <form name="form" id="form" action="index.php" method="post">
    ...
    <table border="0" cellpadding="5" cellspacing="1">
      <tr>
        ...
        <td><input type="text" name="searchDT"
            value="<?php
                    // print $param["searchDT"]
                    print htmlspecialchars($param["searchDT"] ?? '');
                    ?>" />
          <a href="javascript:cal1.write();" onChange="cal1.getFormValue(); cal1.hide();">
            <img src="./images/b_calendar.png"></a><span id="cal1"></span>
        </td>
      </tr>
      <tr>
        ...
        <td><input type="text" name="article"
            value="<?php
                    // print $param["article"]
                    print htmlspecialchars($param["article"] ?? '');
                    ?>" /></td>
      </tr>
      <tr>
        <th>住所<span class="red">（必須）</span></th>
        <td><input type="text" name="address"
            value="<?php print htmlspecialchars($param["address"] ?? ''); ?>" /></td>
      </tr>
      <tr>
        <th>駅</th>
        <td><input type="text" name="station"
            value="<?php print htmlspecialchars($param["station"] ?? ''); ?>" /></td>
      </tr>
      <tr>
        <th>徒歩<span class="red">（必須）</span></th>
        <td><input type="text" name="foot"
            value="<?php print htmlspecialchars($param["foot"] ?? ''); ?>" />分</td>
      </tr>
      <tr>
        <th>築年<span class="red">（必須）</span></th>
        <td><input type="text" name="years"
            value="<?php print htmlspecialchars($param["years"] ?? ''); ?>" />年</td>
      </tr>
      <tr>
        <th>階数<span class="red">（必須）</span></th>
        <td><input type="text" name="floor"
            value="<?php print htmlspecialchars($param["floor"] ?? ''); ?>" />階</td>
      </tr>
      <tr>
        <th>専有面積<span class="red">（必須）</span></th>
        <td><input type="text" name="area"
            value="<?php print htmlspecialchars($param["area"] ?? ''); ?>" />㎡</td>
      </tr>
      <tr>
        <th>売主<span class="red">（必須）</span></th>
        <td><input type="text" name="seller"
            value="<?php print htmlspecialchars($param["seller"] ?? ''); ?>" /></td>
      </tr>
      <tr>
        <th>価格<span class="red">（必須）</span></th>
        <td><input type="text" name="price"
            value="<?php print htmlspecialchars($param["price"] ?? ''); ?>" />万円</td>
      </tr>
      <tr>
        <th>備考</th>
        <td><textarea name="note" cols="50" rows="10"><?php print htmlspecialchars($param["note"] ?? ''); ?></textarea></td>
      </tr>
    </table>
