...
?>
  ...
  <form name="form" id="form" action="index.php" method="post">
    ...
    <table border="0" cellpadding="5" cellspacing="1">
      ...
      <tr>
        <th>業者名（よみ）</th>
        <td><input type="text" name="nameFuri" value="<?php print isset($nameFuri) ? $nameFuri : ''; ?>" /></td>
      </tr>
      <tr>
        <th>支店名</th>
        <td><input type="text" name="branch" value="<?php print isset($branch) ? $branch : ''; ?>" /></td>
      </tr>
      <tr>
        <th>支店名（よみ）</th>
        <td><input type="text" name="branchFuri" value="<?php print isset($branchFuri) ? $branchFuri : ''; ?>" /></td>
      </tr>
      <tr>
        <th>郵便番号</th>
        <td><input type="text" name="zip" value="<?php print isset($zip) ? $zip : ''; ?>" /></td>
      </tr>
      <tr>
        <th>住所（都道府県）</th>
        <td><input type="text" name="prefecture" value="<?php print isset($prefecture) ? $prefecture : ''; ?>" /></td>
      </tr>
      <tr>
        <th>住所1（市区町村名）</th>
        <td><input type="text" name="address1" value="<?php print isset($address1) ? $address1 : ''; ?>" /></td>
      </tr>
      <tr>
        <th>住所2（番地・ビル名）</th>
        <td><input type="text" name="address2" value="<?php print isset($aaddress2) ? $address2 : ''; ?>" /></td>
      </tr>
      <tr>
        <th>TEL</th>
        <td><input type="text" name="tel" value="<?php print isset($tel) ? $tel : ''; ?>" /></td>
      </tr>
      <tr>
        <th>FAX</th>
        <td><input type="text" name="fax" value="<?php print isset($fax) ? $fax : ''; ?>" /></td>
      </tr>
      <tr>
        <th>携帯電話</th>
        <td><input type="text" name="mobile" value="<?php print isset($fax) ? $fax : ''; ?>" /></td>
      </tr>
      <tr>
        <th>内装関係</th>
        <td>
          <input type="checkbox" name="interior" value="1"
            <?php
            //  if ($interior == 1) print ' checked="checked"';
            if (isset($interior) && $interior == 1) {
...
function subTradeEditComplete()
{
  ...
  $interior   = isset($_REQUEST['interior']) ? mysqli_real_escape_string($conn, $_REQUEST['interior']) : 0;
  ...
  // subTrade(); // ← 直接一覧表示関数を呼ぶのではなく

  $orderBy = $_REQUEST['orderBy'] ?? 'TRADENO'; // デフォルト値も考慮
  $orderTo = $_REQUEST['orderTo'] ?? 'DESC';    // デフォルト値も考慮

  header("Location: index.php?act=tradeSearch&orderBy=" . urlencode($orderBy) . "&orderTo=" . urlencode($orderTo));
  exit();
}
